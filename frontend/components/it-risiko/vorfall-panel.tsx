...
import { addItVorfall, abschliessenItVorfall, updateItVorfall } from "@/app/(app)/it-risiko/actions";
...
function VorfallForm({ initial, onDone, onCancel }: { initial?: ItSicherheitsvorfall; onDone: () => void; onCancel: () => void }) {
  const [datum, setDatum] = useState((initial?.datum ?? new Date().toISOString()).slice(0, 10));
  const [schweregrad, setSchweregrad] = useState<ItVorfallSchweregrad>(initial?.schweregrad ?? "mittel");
  ...
  const [meldepflichtBaFin, setMeldepflichtBaFin] = useState(initial?.meldepflichtBaFin ?? false);
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        if (initial) {
          await updateItVorfall(initial.id, { schweregrad, beschreibung, betroffeneSysteme, meldepflichtBaFin });
        } else {
  ...
  return (
    <Card id="vorfaelle">
      <CardHeader>
        ...
        {canWrite && !adding && <Button variant="secondary" className="px-2.5 py-1 text-xs" onClick={() => { setEditingId(null); setAdding(true); }}>+ Vorfall</Button>}
      </CardHeader>
      <CardBody>
        ...
        <div className="flex flex-col gap-2.5">
          {items.map((v) => (
            <div key={v.id} className="overflow-hidden rounded-xl border border-border-subtle">
              <div className="grid grid-cols-[4px_1fr_auto] items-stretch gap-3.5">
                ...
                <div className="flex flex-col items-end justify-center gap-1.5 py-3 pr-4">
                  ...
                  <div className="flex gap-1.5">
                    {canWrite && (
                      <Button variant="ghost" className="px-2.5 py-1 text-xs" disabled={adding} onClick={() => { setAdding(false); setEditingId(editingId === v.id ? null : v.id); }}>
                        Bearbeiten
                      </Button>
                    )}
              ...
              {editingId === v.id && (
                <div className="border-t border-border-subtle p-3"><VorfallForm initial={v} onDone={() => setEditingId(null)} onCancel={() => setEditingId(null)} /></div>
              )}
            </div>
          ))}
