...
import { addItAsset } from "@/app/(app)/it-risiko/actions";
...
function AssetForm({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  const [bezeichnung, setBezeichnung] = useState("");
  const [kategorie, setKategorie] = useState<ItAssetKategorie>(KATEGORIEN[0]);
  const [c, setC] = useState<ItSchutzbedarf | "">("");
  const [i, setI] = useState<ItSchutzbedarf | "">("");
  const [a, setA] = useState<ItSchutzbedarf | "">("");
  ...
  return (
    <Card id="assets">
      <CardHeader>
        ...
        {canWrite && !adding && <Button variant="secondary" className="px-2.5 py-1 text-xs" onClick={() => setAdding(true)}>+ Asset</Button>}
      </CardHeader>
      <CardBody>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {items.length === 0 && (
                <tr><td colSpan={5} className="px-3 py-6 text-center text-muted-foreground">Noch keine IT-Assets erfasst.</td></tr>
              )}
            </tbody>
          </table>
        </div>
