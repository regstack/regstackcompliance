...
import { addItAenderung, genehmigenItAenderung, umsetzenItAenderung, zurueckstellenItAenderung } from "@/app/(app)/it-risiko/actions";
import type { ItAenderung, ItAsset } from "@/lib/regstack/it-risiko";

function AenderungForm({ assets, onDone, onCancel }: { assets: ItAsset[]; onDone: () => void; onCancel: () => void }) {
  const [assetId, setAssetId] = useState("");
  const [bezeichnung, setBezeichnung] = useState("");
  const [art, setArt] = useState("");
  const [risikobewertung, setRisikobewertung] = useState("");
  const [rueckabwicklungsplan, setRueckabwicklungsplan] = useState("");
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        await addItAenderung({ assetId, bezeichnung, art, risikobewertung, rueckabwicklungsplan, geplantAm });
        onDone();
      } catch (e) {
...
function AenderungAktionen({ item }: { item: ItAenderung }) {
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <Card id="aenderungen">
      <CardHeader>
        ...
        {canWrite && !adding && <Button variant="secondary" className="px-2.5 py-1 text-xs" onClick={() => setAdding(true)}>+ Änderung</Button>}
      </CardHeader>
      <CardBody>
            ...
            <tbody>
              {items.map((a) => (
                <tr key={a.id} className="border-b border-border-subtle last:border-0">
                  ...
                  {canWrite && <td className="px-3 py-2 text-right"><AenderungAktionen item={a} /></td>}
                </tr>
              ))}
              {items.length === 0 && (
