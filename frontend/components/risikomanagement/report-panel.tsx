...
import { addRmReport, finalizeRmReport, acknowledgeRmReport } from "@/app/(app)/risikomanagement/actions";
import type { RmReport } from "@/lib/regstack/risikomanagement";
...
function ReportForm({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        await addRmReport(reportType, periodFrom, periodTo);
        onDone();
      } catch (e) {
...
export function ReportPanel({
  ...
  return (
    <Card id="bericht">
      <CardHeader>
        <CardTitle>Bericht an die Geschäftsleitung</CardTitle>
            ...
            {reports.map((r) => {
              ...
              return (
                <div key={r.id} className="rounded-lg border border-border-subtle p-4">
                  ...
                  {r.content.kapitalausstattung && (
                    <div className="grid gap-3 text-[13px] sm:grid-cols-2">
                      <div>
                        <div className="mb-1 text-[10.5px] font-semibold uppercase tracking-wide text-graphite-500">Kapitalausstattung / RTF</div>
                        <p className="leading-relaxed text-foreground">{r.content.kapitalausstattung}</p>
                        {r.content.risikolage && (
                          <>
                            <div className="mb-1 mt-3 text-[10.5px] font-semibold uppercase tracking-wide text-graphite-500">Risikolage</div>
                            <p className="leading-relaxed text-foreground">{r.content.risikolage}</p>
                          </>
                        )}
                      </div>
                      {r.content.massnahmen && (
                        <div>
                          <div className="mb-1 text-[10.5px] font-semibold uppercase tracking-wide text-graphite-500">Maßnahmen</div>
                          <p className="leading-relaxed text-foreground">{r.content.massnahmen}</p>
                        </div>
                      )}
                    </div>
                  )}
