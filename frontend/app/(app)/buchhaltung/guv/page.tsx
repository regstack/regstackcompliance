...
import { listIncomeStatements } from "@/lib/regstack/accounting";
...
import { createIncomeStatement } from "@/app/(app)/buchhaltung/actions";

export default async function GuvListPage() {
  const [session, statements] = await Promise.all([getBackendSession(), listIncomeStatements()]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-foreground">Gewinn- und Verlustrechnung</h2>
        {canWrite && <CreateDocumentForm defaultYear={nextYear} onCreate={createIncomeStatement} detailHrefBase="/buchhaltung/guv" />}
      </div>

      {sorted.length === 0 ? (
        <Card className="px-6 py-12 text-center">
          <p className="text-sm text-muted-foreground">Noch keine GuV erfasst.</p>
        </Card>
      ) : (
        <div className="space-y-3">
          {sorted.map((s) => (
            <Link key={s.id} href={`/buchhaltung/guv/${s.id}`}>
