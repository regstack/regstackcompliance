...
| Funktionstrennung zwischen operativer Tätigkeit und deren Prüfung | AT 4.4.3 Tz. 1 (Unabhängigkeit der Revision), Tz. 1 S.2 (Selbstüberprüfung der Revisionsfunktion) | Eigene Rolle `PRUEFER` mit ausschließlich Lesezugriff + Exportfunktionen, keine Schreibrechte in der Rollenmatrix; zusätzlich muss `INTERNE_REVISION` sich Lesezugriff auf jedes andere Modul (Outsourcing, Compliance, Buchhaltung, IKS, Risikomanagement, IT-Risiko/BAIT) aktiv freigeben lassen (`ModuleAccessGrant`), statt ihn implizit über die Rollenmatrix zu haben | `src/middleware/rbac.ts` (`PRUEFER`, `requireAccessGrant`), `src/modules/accessGrants/` | Umgesetzt |
